...
describe('timerReducer', () => {
  ...
  describe('START', () => {
    ...
    it('ignore START si déjà en WORK', () => {
      const state = { ...INITIAL_STATE, phase: 'WORK' as const }
  ...
  describe('RESUME', () => {
    it('passe PAUSED → WORK', () => {
      const state = { ...INITIAL_STATE, phase: 'PAUSED' as const }
  ...
  describe('TICK', () => {
    it('incrémente elapsed en WORK', () => {
      const state = { ...INITIAL_STATE, phase: 'WORK' as const, elapsed: 0 }
  ...
  describe('PHASE_COMPLETE', () => {
    ...
    it('SHORT_BREAK → IDLE', () => {
      const state = { ...INITIAL_STATE, phase: 'SHORT_BREAK' as const }
      const next = timerReducer(state, { type: 'PHASE_COMPLETE' })
      expect(next.phase).toBe('IDLE')
    })

    it('remet elapsed à 0 à chaque changement de phase', () => {
      const state = { ...INITIAL_STATE, phase: 'WORK' as const, elapsed: 500 }
      const next = timerReducer(state, { type: 'PHASE_COMPLETE' })
      expect(next.elapsed).toBe(0)
    })
  })
